...
func TestStringFlag_Validation(t *testing.T) {
	testCases := []struct {
		...
		{
			title:             "duplicate items in case sensitive validation list",
		...
		{
			title:             "duplicate items in case insensitive validation list",
			validationList:    []string{"Green", "Green"},
			ignoreCase:        true,
			setValidationList: true,
			value:             "Pink",
		...
		},
		{
			...
			setValidationList: true,
			value:             "blue",
			expectedError:     "blue is not an acceptable value for --colours. The expected values are Green,Pink.",
		},
		{
