...
func TestFloat32Flag_WithShort(t *testing.T) {
	testCases := []struct {
		title         string
		long, short   string
		expectedLong  string
	...
	for _, tc := range testCases {
		t.Run(tc.title, func(t *testing.T) {
			f := flags.Float32(tc.long, tc.usage).WithShort(tc.short)
...
func TestFloat32Flag_IsRequired(t *testing.T) {
	testCases := []struct {
		title      string
		isRequired bool
	}{
...
func TestFloat32Flag_Validation(t *testing.T) {
	testCases := []struct {
		title             string
		value             string
		expectedValue     float32
		validationCB      func(in float32) error
		setValidationCB   bool
		validationList    []float32
		...
		},
		{
			...
			validationList:    []float32{100.25, 101.60},
			setValidationList: true,
			value:             "102.80",
		...
		},
		{
			...
			validationList:    []float32{100.25, 100.25},
			setValidationList: true,
			value:             "102.80",
		...
		},
		{
			...
			validationList:    []float32{100.25, 101.30, 102.40},
			setValidationList: true,
			value:             "104.10",
		...
		{
			title:             "max",
			validationList:    []float32{math.MaxFloat32},
			setValidationList: true,
			value:             "104.80",
			expectedError:     fmt.Sprintf("104.80 is not an acceptable value for --long. The expected value is %v.", float32(math.MaxFloat32)),
		},
		{
