...
// NewCIDRSlice creates a new CIDR (Classless Inter-Domain Routing) slice flag.
...
func (f *CIDRSliceFlag) Var() *[]CIDR {
	return f.ptr
}

// Get returns the current value of the flag.
func (f *CIDRSliceFlag) Get() []CIDR {
	return f.value
}
...
func (f *CIDRSliceFlag) WithDefault(defaultValue []CIDR) *CIDRSliceFlag {
...
func (f *CIDRSliceFlag) WithValidationCallback(validate func(in CIDR) error) *CIDRSliceFlag {
	f.validate = validate
	return f
}
...
// Remember that setting the valid range will have no effect if a validation callback has been specified.
func (f *CIDRSliceFlag) WithValidRange(valid ...CIDR) *CIDRSliceFlag {
	if len(valid) == 0 {
		return f
	}
...
func (f *CIDRSliceFlag) Key() *Key {
	return f.key
}

func (f *CIDRSliceFlag) set(value []CIDR) {
	f.value = value
	*f.ptr = value
}
