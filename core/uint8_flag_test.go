...
func TestUInt8Flag_ShortName(t *testing.T) {
	testCases := []struct {
		title         string
		long, short   string
		expectedLong  string
	...
	for _, tc := range testCases {
		t.Run(tc.title, func(t *testing.T) {
			f := flags.UInt8(tc.long, tc.usage).WithShort(tc.short)
...
func TestUInt8Flag_IsRequired(t *testing.T) {
	testCases := []struct {
		title      string
		isRequired bool
	}{
...
func TestUInt8Flag_Validation(t *testing.T) {
	testCases := []struct {
		title             string
		value             string
		expectedValue     uint8
		validationCB      func(in uint8) error
		setValidationCB   bool
		validationList    []uint8
		...
		},
		{
			...
			validationList:    []uint8{100, 101, 102},
			setValidationList: true,
			value:             "104",
		...
		{
			title:             "min and max",
			validationList:    []uint8{0, math.MaxUint8},
			setValidationList: true,
			value:             "104",
