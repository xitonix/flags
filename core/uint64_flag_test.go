...
func TestUInt64(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.title, func(t *testing.T) {
			f := flags.UInt64(tc.long, tc.usage)
			checkFlagInitialState(t, f, "uint64", tc.expectedUsage, tc.expectedLong, "")
			checkFlagValues(t, uint64(0), f.Get(), f.Var())
		})
	}
}

func TestUInt64Flag_WithShort(t *testing.T) {
	testCases := []struct {
		title         string
		long, short   string
		expectedLong  string
	...
	for _, tc := range testCases {
		t.Run(tc.title, func(t *testing.T) {
			f := flags.UInt64(tc.long, tc.usage).WithShort(tc.short)
			checkFlagInitialState(t, f, "uint64", tc.expectedUsage, tc.expectedLong, tc.expectedShort)
			checkFlagValues(t, uint64(0), f.Get(), f.Var())
		})
	}
}

func TestUInt64Flag_WithKey(t *testing.T) {
	testCases := []struct {
		title       string
		key         string
		expectedKey string
	...
	for _, tc := range testCases {
		t.Run(tc.title, func(t *testing.T) {
			f := flags.UInt64("long", "usage").WithKey(tc.key)
			actual := f.Key().String()
...
func TestUInt64Flag_WithDefault(t *testing.T) {
	testCases := []struct {
		title                string
		defaultValue         uint64
		expectedDefaultValue uint64
	}{
		{
		...
		},
		{
			...
			expectedDefaultValue: math.MaxUint64,
		},
	}

	for _, tc := range testCases {
		t.Run(tc.title, func(t *testing.T) {
			f := flags.UInt64("long", "usage").WithDefault(tc.defaultValue)
			actual := f.Default()
...
func TestUInt64Flag_Hide(t *testing.T) {
	testCases := []struct {
		title    string
		isHidden bool
	}{
...
func TestUInt64Flag_IsDeprecated(t *testing.T) {
	testCases := []struct {
		title        string
		isDeprecated bool
	}{
...
func TestUInt64Flag_IsRequired(t *testing.T) {
	testCases := []struct {
		title      string
		isRequired bool
	}{
...
func TestUInt64Flag_Set(t *testing.T) {
	testCases := []struct {
		title         string
		value         string
		...
		},
		{
			title:         "invalid value",
			value:         "abc",
			expectedError: "is not a valid uint64 value",
			expectedValue: 0,
		},
		{
			title:         "max uint64",
			value:         fmt.Sprintf("%d", uint64(math.MaxUint64)),
...
func TestUInt64Flag_Validation(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			setValidationList: true,
			value:             "104",
			expectedError:     fmt.Sprintf("104 is not an acceptable value for --long. The expected values are %v,%v.", 0, uint64(math.MaxUint64)),
		},
		{
			title:             "duplicate items in the validation list",
			validationList:    []uint64{100, 100},
			setValidationList: true,
			value:             "101",
			expectedError:     "101 is not an acceptable value for --long. The expected value is 100.",
		},
...
func TestUInt64Flag_ResetToDefault(t *testing.T) {
	testCases := []struct {
		title                   string
		value                   string
		expectedValue           uint64
		defaultValue            uint64
		expectedAfterResetValue uint64
		expectedError           string
		setDefault              bool
	}{
		{
