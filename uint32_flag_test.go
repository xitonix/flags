...
func TestUInt32(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.title, func(t *testing.T) {
			f := flags.UInt32(tc.long, tc.usage)
			checkFlagInitialState(t, f, "uint32", tc.expectedUsage, tc.expectedLong, "")
			checkFlagValues(t, uint32(0), f.Get(), f.Var())
		})
	}
}

func TestUInt32Flag_WithShort(t *testing.T) {
	testCases := []struct {
		title         string
		long, short   string
		expectedLong  string
	...
	for _, tc := range testCases {
		t.Run(tc.title, func(t *testing.T) {
			f := flags.UInt32(tc.long, tc.usage).WithShort(tc.short)
			checkFlagInitialState(t, f, "uint32", tc.expectedUsage, tc.expectedLong, tc.expectedShort)
			checkFlagValues(t, uint32(0), f.Get(), f.Var())
		})
	}
}

func TestUInt32Flag_WithKey(t *testing.T) {
	testCases := []struct {
		title       string
		key         string
		expectedKey string
	...
	for _, tc := range testCases {
		t.Run(tc.title, func(t *testing.T) {
			f := flags.UInt32("long", "usage").WithKey(tc.key)
			actual := f.Key().String()
...
func TestUInt32Flag_WithDefault(t *testing.T) {
	testCases := []struct {
		title                string
		defaultValue         uint32
		expectedDefaultValue uint32
	}{
		{
		...
		{
			title:                "max uint32",
			defaultValue:         math.MaxUint32,
			expectedDefaultValue: math.MaxUint32,
		},
	}

	for _, tc := range testCases {
		t.Run(tc.title, func(t *testing.T) {
			f := flags.UInt32("long", "usage").WithDefault(tc.defaultValue)
			actual := f.Default()
...
func TestUInt32Flag_Hide(t *testing.T) {
	testCases := []struct {
		title    string
		isHidden bool
	}{
...
func TestUInt32Flag_IsDeprecated(t *testing.T) {
	testCases := []struct {
		title        string
		isDeprecated bool
	}{
...
func TestUInt32Flag_IsRequired(t *testing.T) {
	testCases := []struct {
		title      string
		isRequired bool
	}{
...
func TestUInt32Flag_Set(t *testing.T) {
	testCases := []struct {
		title         string
		value         string
		...
		},
		{
			title:         "invalid value",
			value:         "abc",
			expectedError: "is not a valid uint32 value",
			expectedValue: 0,
		},
		{
			title:         "max uint32",
			value:         fmt.Sprintf("%d", uint32(math.MaxUint32)),
...
func TestUInt32Flag_Validation(t *testing.T) {
	testCases := []struct {
		title             string
		value             string
		expectedValue     uint32
		validationCB      func(in uint32) error
		setValidationCB   bool
		validationList    []uint32
		...
		},
		{
			...
			validationList:    []uint32{100, 101, 102},
			setValidationList: true,
			value:             "104",
		...
		{
			title:             "min and max",
			validationList:    []uint32{0, math.MaxUint32},
			setValidationList: true,
			value:             "104",
			expectedError:     fmt.Sprintf("104 is not an acceptable value for --long. The expected values are %v,%v.", 0, math.MaxUint32),
		},
		{
			title:             "duplicate items in the validation list",
			validationList:    []uint32{100, 100},
			setValidationList: true,
			value:             "101",
			expectedError:     "101 is not an acceptable value for --long. The expected value is 100.",
		},
...
func TestUInt32Flag_ResetToDefault(t *testing.T) {
	testCases := []struct {
		title                   string
		value                   string
		expectedValue           uint32
		defaultValue            uint32
		expectedAfterResetValue uint32
		expectedError           string
		setDefault              bool
	}{
		{
