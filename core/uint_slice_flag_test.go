...
func TestUIntSliceFlag_WithShort(t *testing.T) {
	testCases := []struct {
		title         string
		long, short   string
		expectedLong  string
	...
	for _, tc := range testCases {
		t.Run(tc.title, func(t *testing.T) {
			f := flags.UIntSlice(tc.long, tc.usage).WithShort(tc.short)
...
func TestUIntSliceFlag_IsRequired(t *testing.T) {
	testCases := []struct {
		title      string
		isRequired bool
	}{
...
func TestUIntSliceFlag_Set(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			title:         "invalid value",
			value:         " invalid ",
			expectedError: "'invalid' is not a valid []uint value for --long",
			expectedValue: empty,
		},
		{
