...
func TestInt16Flag_WithShort(t *testing.T) {
	testCases := []struct {
		title         string
		long, short   string
		expectedLong  string
	...
	for _, tc := range testCases {
		t.Run(tc.title, func(t *testing.T) {
			f := flags.Int16(tc.long, tc.usage).WithShort(tc.short)
...
func TestInt16Flag_IsRequired(t *testing.T) {
	testCases := []struct {
		title      string
		isRequired bool
	}{
...
func TestInt16Flag_Validation(t *testing.T) {
	testCases := []struct {
		title             string
		value             string
		expectedValue     int16
		...
		},
		{
			...
			validationList:    []int16{100, 101, 102},
			setValidationList: true,
			value:             "104",
		...
		},
		{
			...
			setValidationList: true,
			value:             "104",
			expectedError:     fmt.Sprintf("104 is not an acceptable value for --long. The expected values are %v,%v.", math.MinInt16, math.MaxInt16),
		},
		{
