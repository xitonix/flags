...
func TestInt32P(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.title, func(t *testing.T) {
			f := flags.Int32P(tc.long, tc.usage, tc.short)
...
func TestInt32Flag_Validation(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			setValidationList: true,
			value:             "104",
			expectedError:     fmt.Sprintf("104 is not an acceptable value for --long. The expected values are %v and %v.", math.MinInt32, math.MaxInt32),
		},
		{
